...
class HSVColorDetector:
    ...
    def _get_hsv_bounds(self, color: str):
        ...
        c = self.config[color]
        hmin = int(round(float(c["hmin"]) / 2.0))
        hmax = int(round(float(c["hmax"]) / 2.0))
        smin = int(round(float(c["smin"])))
        smax = int(round(float(c["smax"])))
        vmin = int(round(float(c["vmin"])))
        vmax = int(round(float(c["vmax"])))

        # OpenCV HSV is H:0-179, S/V:0-255. Keep the original ordering so red
        # wrap-around ranges (hmin > hmax) still work.
        hmin = max(0, min(179, hmin))
        hmax = max(0, min(179, hmax))
        smin = max(0, min(255, smin))
        smax = max(0, min(255, smax))
    ...
    def _find_largest(self, image, lower, upper):
        ...
        if lower[0] > upper[0]:  # hue wrap (e.g. red)
            lower1 = np.array([0, int(lower[1]), int(lower[2])], dtype=np.uint8)
            upper1 = upper.astype(np.uint8)
            lower2 = lower.astype(np.uint8)
